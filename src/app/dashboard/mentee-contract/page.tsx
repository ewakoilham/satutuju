...
import MenteeIdentityForm from "@/components/contract/MenteeIdentityForm";
...
import type { PartialMenteeIdentity } from "@/lib/mentee-contract-template";
import {
...
/**
 * Phase 18 — Mentee contract signing flow. Direct parallel of the mentor
 * page (`src/app/dashboard/contract/page.tsx`), wired to the mentee
 * endpoints + identity form. Three-step wizard:
 *   1. Identity (MenteeIdentityForm — 7 fields, no NPWP)
 *   2. Read full contract (scroll to end gate)
 *   3. Draw/upload signature + 3 confirmations
 *
 * After sign, shows the audit panel with download PDF + re-sign affordance.
 * Re-sign reuses the same flow but skips back to Step 3 (identity stays
 * frozen on the snapshot).
 */
...
interface ContractApiResponse {
  ...
  identity: PartialMenteeIdentity;
...
export default function MenteeContractPage() {
  ...
  const reload = useCallback(async () => {
    setReloading(true);
    try {
      const res = await fetch("/api/mentee-contract", {
        credentials: "include",
        cache: "no-store",
      });
  ...
  if (!user || user.role !== "mentee") {
    return (
      <div className="p-8">
        <p className="text-text-muted">Halaman ini hanya untuk mentee.</p>
      </div>
    );
  }
      ...
      <div className="px-4 md:px-8 py-10 max-w-2xl mx-auto">
        <div className="rounded-xl border border-danger/40 bg-danger-light/40 p-5">
          ...
          <p className="mt-2 text-xs text-text-muted">
            Penyebab paling umum: tabel <code>MenteeContract</code> belum
            dibuat di database. Pastikan migrasi Phase 18 sudah dijalankan,
            lalu muat ulang halaman ini.
          </p>
          <button type="button" className="mt-3 btn-ghost" onClick={reload}>
            Coba Lagi
          </button>
        </div>
      </div>
      ...
      <header className="mb-8">
        ...
        {resigning ? (
          ...
          <p className="mt-2 text-sm text-text-muted">
            Nomor kontrak:{" "}
            <span className="font-semibold">{data.contract.contractNumber}</span>
          ...
          <p className="mt-2 text-sm text-text-muted">
            Lengkapi data identitas, baca seluruh kontrak, lalu tanda tangani
            secara digital. Tanda tangan elektronik ini sah berdasarkan UU No.
        ...
        {status === "VOID" && data.contract?.voidReason && (
          <div className="mt-4 rounded-lg border border-danger/40 bg-danger-light/60 px-4 py-3 text-sm">
            <p className="font-semibold text-danger">
              Kontrak ini dibatalkan oleh admin.
            </p>
            <p className="mt-1 text-foreground">
              Alasan: {data.contract.voidReason}
            </p>
            <p className="mt-1 text-text-muted">
              Anda dapat menandatangani ulang — kontrak baru akan menggunakan
              nomor seri berikutnya.
            </p>
          </div>
        )}
      </header>
          ...
          {step === 2 && (
            ...
            >
              ...
              <label className="mt-5 flex items-start gap-3 text-sm">
                ...
                />
                <span>
                  Saya telah membaca seluruh isi Perjanjian Layanan Mentoring
                  Mentee di atas.
                  {!scrolledToEnd && (
          ...
          {step === 3 && (
            ...
            >
              ...
              <div className="space-y-3 mb-6 text-sm">
                ...
                >
                  Saya bertindak untuk dan atas nama diri sendiri, dan memiliki
                  kapasitas hukum yang sah untuk menandatangani Perjanjian ini.
                </ConfirmRow>
                <ConfirmRow
                  checked={confirmNoConflict}
                  onChange={setConfirmNoConflict}
                >
                  Penandatanganan ini tidak melanggar perjanjian lain atau
                ...
                >
                  Seluruh informasi identitas yang saya berikan adalah benar,
...
function SignedView({
  ...
  async function regenerate() {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/mentee-contract/regenerate-pdf", {
        method: "POST",
        credentials: "include",
      });
      const json = await res.json();
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-1 text-sm text-text-muted">
                ...
                terbaru. Klik tombol &quot;Tanda Tangan Ulang&quot; di samping
                untuk melanjutkan.
              </p>
      ...
      <section className="rounded-2xl border border-success/40 bg-success-light/40 p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Terima kasih — perjanjian layanan mentoring Anda dengan Satu Tuju
              kini telah resmi.
            </p>
          ...
          <div className="flex flex-col items-stretch sm:items-end gap-2 self-start">
            ...
            {err && (
              <span className="text-xs text-danger max-w-[14rem] text-right">
                {err}
              </span>
            )}
          </div>
