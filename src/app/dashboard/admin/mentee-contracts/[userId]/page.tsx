...
export default function AdminContractDetailPage() {
  ...
  const reload = useCallback(async () => {
    ...
    const res = await fetch(`/api/admin/mentee-contracts/${targetUserId}`, {
      credentials: "include",
    });
    if (!res.ok) {
  ...
  const stale = isMenteeContractStale(data.contract, data.currentVersion);
  const c = data.contract;
  const identity =
    (c?.identitySnapshot as PartialMenteeIdentity | null) ?? data.identity;

  return (
    <div className="space-y-6">
      <Link
        href="/dashboard/admin/mentee-contracts"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-foreground">
                Mentee menandatangani versi lama (v{c.templateVersion}). Versi
                aktif sekarang adalah v{data.currentVersion}.
              </p>
              <p className="mt-1 text-sm text-text-muted">
                Mentee sudah dapat melihat banner peringatan + notifikasi dan
      ...
      <section className="rounded-2xl border border-border bg-surface-elevated p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {status === "SIGNED" && (
              <a
                href={`/api/mentee-contract/pdf?userId=${targetUserId}`}
                target="_blank"
                rel="noopener"
      ...
      <Modal open={voidOpen} onClose={() => setVoidOpen(false)} size="md">
        <div className="px-6 py-5">
          ...
          <p className="mt-1 text-sm text-text-muted">
            Pembatalan akan menjadikan status kontrak <strong>VOID</strong>. Mentee
          ...
          <textarea
            ...
            placeholder="Mis. data identitas keliru, mentee minta perubahan, dsb."
          />
