...
export default function ResourcesPage() {
  ...
  // ════════════════════════════════════════════════════════════════
  //  MENTEE VIEW — per-session learner library (mentee design handoff
  //  "Materi Mentee.html"). STRUCTURE from real data only: the mentee's
  //  own pairing sessions drive the modules + lock states; materials are
  //  the real (role-visible) catalog mapped by phase. No invented content —
  //  sessions with no material yet show an honest "sedang disiapkan" note.
  //  Reuses the existing materi-* / db-pill / hero-shelf CSS.
  // ════════════════════════════════════════════════════════════════
  if (role === "mentee") {
    const pairing = pairings.find((p) => p.status === "active") || pairings[0] || null;
    const mySessions = [...(pairing?.sessions || [])].sort((a, b) => a.sessionNum - b.sessionNum);
    const mentorFirst = firstName(pairing?.mentor?.name || "");
    const isDoneS = (s: RecSession) => s.status === "completed" || !!s.mentorSubmittedAt;
    const completedCount = mySessions.filter(isDoneS).length;
    const totalSessions = mySessions.length || CURRICULUM.length;
    const firstActive = mySessions.find((s) => !isDoneS(s)) || null;
    const firstActiveNum = firstActive?.sessionNum ?? (mySessions[mySessions.length - 1]?.sessionNum ?? 1);

    // Mentee-visible materials + the active filters (type / saved / search).
    const matchFilters = (m: Material) => {
      if (types.size > 0 && !types.has(m.type)) return false;
      if (bookmarkedOnly && !bookmarks.has(m.id)) return false;
      if (search.trim()) {
        const q = search.toLowerCase();
    ...
    const menteeMats = MATERIALS.filter((m) => m.roles === null || m.roles.includes("mentee"));
    const filteredMats = menteeMats.filter(matchFilters);
    const sessionNums = new Set(mySessions.map((s) => s.sessionNum));

    // Placement: a material with `sessionN` files under that exact session.
    // Without it, a phase-specific material attaches to the EARLIEST session of
    // its phase (so it shows once, not on every same-phase session).
    const phaseFirstNum: Record<string, number> = {};
    for (const s of mySessions) if (!(s.phase in phaseFirstNum)) phaseFirstNum[s.phase] = s.sessionNum;
    const matsForSession = (s: RecSession) =>
      filteredMats.filter((m) =>
        (m.sessionN != null && m.sessionN === s.sessionNum) ||
        (m.sessionN == null && m.phase !== "all" && m.phase === s.phase && phaseFirstNum[s.phase] === s.sessionNum)
      );
    // "Materi umum" = phase-agnostic resources, plus any session-tagged material
    // whose session this pairing doesn't have yet (so nothing gets hidden).
    const generalMats = filteredMats.filter((m) =>
      (m.sessionN == null && m.phase === "all") ||
      (m.sessionN != null && !sessionNums.has(m.sessionN))
    );

    const totalVisible = menteeMats.filter((m) => !m.locked).length;
    const activePhase = firstActive?.phase;
    let featuredMat: Material | undefined;
    if (activePhase) featuredMat = menteeMats.find((m) => !m.locked && m.phase === activePhase);
    if (!featuredMat) featuredMat = menteeMats.find((m) => m.id === "curriculum") || menteeMats[0];
    ...
    const renderCard = (m: Material) => {
      ...
      const inner = (
        <>
          ...
          <div className="top">
            <div className="icon"><Icon name={m.icon} size={18} /></div>
            {!m.locked && (
              <button
                type="button"
      ...
      return m.href && !m.locked ? (
        <Link key={m.id} href={m.href} className={`materi-card tone-${m.tone}`} onClick={() => recordOpen(m.id)}>{inner}</Link>
      ) : (
        <div key={m.id} className={`materi-card tone-${m.tone}${m.locked ? " locked" : ""}`}>{inner}</div>
      );
    };

    return (
      <>
        <div className="page-head" style={{ marginBottom: 8 }} data-tour-screen="resources">
          <div>
            <div className="sesi-crumb">Materi</div>
            <h1 className="sesi-title">
              Materi <span className="lede">buat belajar mandiri.</span>
            </h1>
            <p className="sesi-sub">
              Panduan & materi tersusun per sesi{mentorFirst ? `, dikurasi ${mentorFirst}` : ""}. Buka sebelum sesi biar waktu bareng mentor lebih maksimal. Sebagian materi masih disiapkan.
            </p>
          </div>
        </div>

        {/* Lanjutkan — featured material for the current session's phase */}
        {featuredMat && (
          <section className="hero-shelf">
            <div className="hero-cover">
              <div className="num">Lanjutkan{firstActive ? ` · Sesi ${firstActiveNum}` : ""}</div>
              <div>{featuredMat.title}</div>
              <div className="corner-fold" />
            </div>
            <div className="hero-meta">
              <span className="db-pill">Untuk kamu</span>
              <h2>{featuredMat.title}.</h2>
              <p>{featuredMat.description}</p>
              <div className="actions">
                {featuredMat.href ? (
                  <Link href={featuredMat.href} className="db-btn db-btn-primary" onClick={() => recordOpen(featuredMat.id)}>
                ...
                {firstActive && (
                  <Link href={`/dashboard/sesi/${firstActive.id}`} className="db-btn db-btn-outline">
                    Lihat Sesi {firstActiveNum}
                  </Link>
                )}
              </div>
            </div>
        ...
        <div className="materi-filter-row">
          <div className="materi-search">
            ...
            <input placeholder="Cari materi — coba 'template' atau 'essay'…" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="filter-chips">
          {TYPE_CHIPS.map((t) => (
            <button type="button" key={t.key} className={`db-pill ${types.has(t.key) ? "on" : ""}`} onClick={() => toggleType(t.key)}>
              {t.label}
            </button>
          ))}
          <button type="button" className={`db-pill ${bookmarkedOnly ? "on" : ""}`} onClick={() => setBookmarkedOnly((x) => !x)}>
            ★ Tersimpan
          </button>
        </div>

        <div className="materi-two-col">
          {/* MAIN — per-session modules */}
          <div>
            {mySessions.length === 0 ? (
              <div className="materi-card" style={{ padding: 24 }}>
                <h3 style={{ margin: 0 }}>Belum ada sesi</h3>
                <p className="desc" style={{ marginTop: 6 }}>
                  Kamu akan dipasangkan dengan mentor sebentar lagi. Sementara itu, materi umum di bawah bisa kamu buka.
                </p>
              </div>
            ) : (
              mySessions.map((s) => {
                const done = isDoneS(s);
                const isNext = firstActive?.sessionNum === s.sessionNum;
                const locked = !done && s.sessionNum > firstActiveNum;
                const mats = matsForSession(s);
                return (
                  <section className="section" key={s.id} style={{ marginTop: 20 }}>
                    <div className="section-head">
                      <h2 style={{ fontSize: 17 }}>
                        Sesi {String(s.sessionNum).padStart(2, "0")} · {s.topic || PHASE_LABEL[s.phase]}
                      </h2>
                      <span className="meta">
                        {done ? "Selesai" : isNext ? "Sesi berikutnya" : locked ? "Terkunci" : PHASE_LABEL[s.phase]}
                      </span>
                    </div>
                    {locked ? (
                      <div className="materi-card" style={{ padding: 18, opacity: 0.75 }}>
                        <div className="desc">🔒 Materi kebuka setelah kamu menyelesaikan sesi sebelumnya.</div>
                      </div>
                    ) : mats.length === 0 ? (
                      <div className="materi-card" style={{ padding: 18 }}>
                        <div className="desc">Materi untuk sesi ini sedang disiapkan.</div>
                      </div>
                    ) : (
                      <div className="materi-grid">{mats.map(renderCard)}</div>
                    )}
                  </section>
                );
              })
            ...
            {/* Materi umum — phase-agnostic resources */}
            {generalMats.length > 0 && (
              <section className="section" style={{ marginTop: 20 }}>
                <div className="section-head">
                  <h2 style={{ fontSize: 17 }}>Materi umum</h2>
                  <span className="meta">berlaku semua sesi</span>
                </div>
                <div className="materi-grid">{generalMats.map(renderCard)}</div>
              </section>
            )}
          </div>
          ...
          <div>
            <div className="side-card prog-card">
              <div className="prog-head">
                <div>
                  <span className="eyebrow">Progress materi</span>
                  <div className="m">{completedCount} / {totalSessions} sesi selesai</div>
                </div>
                <span className="db-badge primary">{totalVisible} materi</span>
              </div>
              <div className="seg">
                {Array.from({ length: totalSessions }).map((_, i) => (
                  <i key={i} className={i < completedCount ? "done" : i === completedCount ? "now" : ""} />
                ))}
              </div>
              <p>Materi kebuka bertahap mengikuti progress sesi kamu. Selesaikan sesi berjalan untuk membuka modul berikutnya.</p>
            </div>

            <div className="side-card" style={{ padding: 20 }}>
              <span className="eyebrow" style={{ display: "block", marginBottom: 12 }}>Disimpan</span>
              {[...bookmarks].map((id) => MATERIALS.find((m) => m.id === id)).filter((m): m is Material => !!m && (m.roles === null || m.roles.includes("mentee"))).length === 0 ? (
                <p className="desc" style={{ margin: 0 }}>Belum ada materi tersimpan. Tap ★ di kartu mana pun buat simpan.</p>
              ) : (
                <div className="se-rail-list">
                  {[...bookmarks]
                    .map((id) => MATERIALS.find((m) => m.id === id))
                    .filter((m): m is Material => !!m && (m.roles === null || m.roles.includes("mentee")))
                    .map((m) =>
                      m.href ? (
                        <Link key={m.id} href={m.href} className="se-rail-row" style={{ textDecoration: "none" }} onClick={() => recordOpen(m.id)}>
                          <div className="se-rail-info"><div className="se-rail-title">{m.title}</div><div className="se-rail-meta">{m.label}</div></div>
                        </Link>
                      ) : (
                        <div key={m.id} className="se-rail-row"><div className="se-rail-info"><div className="se-rail-title">{m.title}</div><div className="se-rail-meta">{m.label}</div></div></div>
                      ),
                    )}
                </div>
              )}
            ...
            <div className="tip">
              <span className="tag">Tip dari {mentorFirst || "mentor"}</span>
              <h4>Buka materi sebelum sesi, bukan sesudah.</h4>
              <p>10 menit nonton/baca sebelum ketemu bikin waktu bareng mentor kepakai buat hal yang nggak bisa kamu cari sendiri.</p>
            </div>
          </div>
        </div>
      </>
